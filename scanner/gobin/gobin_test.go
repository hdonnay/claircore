...
func TestScanner(t *testing.T) {
	...
	// Build a go binary.
	outname := filepath.Join(tmpdir, "cctool")
	if err := exec.CommandContext(ctx, "go", "build", "-o", outname, "../../cmd/cctool").Run(); err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range vs {
		...
		case v.PackageDB != "go:usr/bin/cctool":
