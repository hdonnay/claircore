...
type Updater interface {
	Update(context.Context, *http.Client, UpdateOperation) error
}

type UpdateOperation interface {
	// Meta:
	PreviousOp(ctx context.Context) (*UpdateOperationMetadata, map[string]string, error)
	SetOpAttr(ctx context.Context, key, value string) error
	SetOpStatus(ctx context.Context, err error) error
...
type UpdateOperationMetadata struct {
	ID [16]byte // UUID
	At time.Time
}
